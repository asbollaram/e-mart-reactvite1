import React from 'react';
import { menData } from '../assets/data/men';
import { useParams } from 'react-router-dom';
import Navbar from '../e-Mart/compponets/Navbar';
import { useCart } from '../e-Mart/contextCart/ContextCart';

const MenAddtoCartDetails = () => {
  const { id } = useParams();

  const { addToCart, cartItems } = useCart();

  const menProduct = menData.find((item) => item.id === id);

  return (
    <div>
      <Navbar />
      <div className="cartproductcontainer">
        <div className="cartHeading">
          <h2>Product Details</h2>
        </div>
        <div className="cartproductDetails">
          <div className="cartImage">
            <img src={menProduct.image} alt="Menfunshion" />
          </div>
          <div className="rightdis">
            <div className="cartName  space">
              <h1>{menProduct.brand}</h1>
            </div>
            <div className="cartModel space">
              <h3>{menProduct.model}</h3>
            </div>
            <div className="cartprice space">
              <h4>{menProduct.price}</h4>
            </div>
            <div className="cartDisc space">
              <p>{menProduct.description}</p>
              <button
                className="cartButton"
                onClick={() => addToCart(menProduct)}
              >
                Add to Cart
              </button>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
};

export default MenAddtoCartDetails;
